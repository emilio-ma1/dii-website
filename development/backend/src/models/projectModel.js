...
const ProjectModel = {
  /**
   * Obtiene todos los proyectos registrados, ordenados por año descendente.
   *
  ...
  getAll: async () => {
    try {
      const query = 'SELECT * FROM projects ORDER BY year DESC, id DESC;';
  ...
  /**
   * Obtiene un proyecto específico con todos sus autores anidados en formato JSON.
   * Utiliza LEFT JOIN y json_agg para agrupar profesores y egresados en un solo objeto.
   *
   * @param {number|string} projectId - El ID único del proyecto a buscar.
   * @returns {Promise<object|null>} El objeto del proyecto con arrays de 'professors' y 'alumni', o null.
   * @throws {Error} Si ocurre un problema al ejecutar la consulta SQL.
   */
  getByIdWithAuthors: async (projectId) => {
    try {
      // Esta consulta agrupa las relaciones directamente en la base de datos
      const query = `
        SELECT 
          p.*,
          COALESCE(
